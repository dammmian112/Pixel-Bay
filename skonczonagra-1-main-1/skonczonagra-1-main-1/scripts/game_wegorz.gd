...
@export var snake_scene : PackedScene = preload("res://scenes/snake_segment.tscn")
...
func new_game():
	...
	score = 0
	$hud_wegorz.get_node("ScoreLabel").text = "Wynik: " + str(score)
	move_direction = up
	can_move = true
	generate_snake()
	move_food()
...
func check_food_eaten():
	...
	if snake_data[0] == food_pos:
		var food_pickup_sound = AudioStreamPlayer.new()
		food_pickup_sound.stream = load("res://assets/spirites/sounds/pythonupgradee.mp3")
		add_child(food_pickup_sound)
...
func end_game():
	$MoveTimer.stop()
	game_started = false
	get_tree().change_scene_to_file("res://scenes/smierc_wegorz_git.tscn")
